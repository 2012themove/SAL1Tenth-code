...
class RoutePoseProvider(Node):
    ...
    def __init__(self):
        super().__init__('route_pose_provider')
        self.srv = self.create_service(GetRoutePoses, 'get_route_poses_car1', self.get_route_poses_callback)        # CHANGE
        self.srv2 = self.create_service(GetRoutePoses, 'get_route_poses_car2', self.get_route_poses_callback_car2)

        self.declare_parameter('pose_filename', 'pose_list_car1.txt')  # default file name
        self.declare_parameter('pose_filename_car2', 'pose_list_car2.txt') 
        ...
        self.pose_filename_car2 = self.get_parameter('pose_filename_car2').value
        self.state_defs = self.get_parameter('state_defs').value

        self.read_in_route = False
        self.route_poses = []
        self.route_poses_car2 = []
    ##END OF CONSTRUCTOR
    #####################################################################################################
    ...
    def get_route_poses_callback_car2(self, request, response):
        self.get_logger().info("Incoming request for route.")
        ...
        num_poses = len(self.route_poses_car2)
        ...
        for i, route_pose in enumerate(self.route_poses_car2):
            response.mypose[i].position.x = route_pose.pt[0]
            response.mypose[i].position.y = route_pose.pt[1]
            response.mypose[i].position.z = route_pose.pt[2]
            response.mypose[i].orientation.w = math.cos(route_pose.heading_rad / 2.0)
...
def main(args=None):
    ...
    ##CAR1 STUFF
    #######################################################################################################
    if route_pose_provider.pose_filename == 'pose_list_car1.txt':
    ...
    ##CAR2 STUFF
    ########################################################################################################
    if route_pose_provider.pose_filename_car2 == 'pose_list_car2.txt':
        filename = get_package_share_directory('gps_nav') + '/data/' + route_pose_provider.pose_filename_car2
    else:
        filename = route_pose_provider.pose_filename_car2
    ...
    for route_pose in route_poses:
        ...
        route_pose_provider.route_poses_car2.append(route_pose_class(np.array([easting, northing, 0.0]), heading_rad, int(state)))
 
    ########################################################################################################                
    
    ##confirm that both routes are ready to read
